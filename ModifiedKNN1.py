			...
			'''for key in self.rank.keys() :
				if abs(key - self.deg) < minimum :
					minimum = key'''
                        ...
                        for it in xrange(3) :
                                d = difference[it][1]  #THIS GIVES US THE DEGREE WHICH CAN HELP US IN GETTING THE RANK CORRESPONDING TO THE DEGREE
                                local_rank = local_rank + (self.rank[d]*(3-it))
                        local_rank = float(local_rank) / 6.0
			print 'MIN : {}\n'.format(local_rank)
			self.prediction += local_rank
		return ((float(self.prediction))/(self.neighbours))
